...
if ($result){
    if ($result->num_rows > 0) {
        ...
        $_SESSION['error'] = "Aspirasi tidak ditemukan atau Anda tidak memiliki akses ke aspirasi tersebut.";
        header("Location: dashboard.php");
        exit();
    }
} else {
    $_SESSION['error'] = "Gagal menyiapkan kueri DETAIL ASPIRASI. Error: " . $conn->error;
    header("Location: dashboard.php");
    exit();
}
...
<body>
    ...
    <div class="dashboard-content">
        ...
        <h1 class="mb-4 fw-bold">
            <?php if (!$is_editable): ?>
                <?= htmlspecialchars($aspirasi['title']) ?>
            <?php else: ?>
                <input type="text" class="form-control form-control-lg fw-bold" name="title" value="<?= htmlspecialchars($aspirasi['title']) ?>" required>
            <?php endif; ?>
        </h1>
        ...
        <div class="detail-card">
            <div class="row">
                <div class="col-lg-8">
                    ...
                    <?php else: ?>
                        <textarea class="form-control lead" name="content" rows="8" required><?= htmlspecialchars($aspirasi['content']) ?></textarea>
                        <div class="form-text text-danger mt-2">Anda hanya bisa mengedit saat status masih Pending.</div>
                    <?php endif; ?>
                                ...
                                <select class="form-select" name="category" required>
                                    <?php foreach ($categories as $cat): ?>
                                        <option value="<?= htmlspecialchars($cat) ?>"
                                            <?= ($aspirasi['category'] === $cat) ? 'selected' : '' ?>>
                ...
                <?php if ($is_editable): ?>
                    <div class="col-12 mt-4 text-end">
                        <button type="submit" name="submit_edit" class="btn btn-lg btn-success">
                ...
                <?php else: ?>
                    <div class="col-12 mt-4 text-end">
                        <button type="button" class="btn btn-lg btn-secondary" disabled>
                            <i class="bi bi-lock me-2"></i> Tidak Dapat Diedit
                        </button>
                    </div>
                <?php endif; ?>
